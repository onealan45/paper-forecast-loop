...
| Research evidence | Supports baselines, research datasets, backtests, reports, walk-forward, research gates, PR6 strategy-card / experiment-trial registry, PR7 locked-evaluation / leaderboard hard-gate artifacts, PR8 paper-shadow outcome learning, PR9 agenda/autopilot loop records, PR14 revision retest scaffolds, PR16 retest task planning, PR18 run logs, and PR19 visible run-log evidence. | Needs strategy generation, self-evolving skills, CPCV-like paths, PBO, DSR, bootstrap, and parameter stability. |
