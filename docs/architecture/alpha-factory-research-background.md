...
| Research evidence | Supports baselines, research datasets, backtests, reports, walk-forward, research gates, PR6 strategy-card / experiment-trial registry, PR7 locked-evaluation / leaderboard hard-gate artifacts, PR8 paper-shadow outcome learning, PR9 agenda/autopilot loop records, PR14 revision retest scaffolds, PR16 retest task planning, PR18 run logs, PR19 visible run-log evidence, PR20 protocol-lock execution, PR21 baseline execution, PR22 holdout backtest execution from locked split windows, PR23 walk-forward execution from locked full split windows, PR24 PASSED retest trial recording from existing evidence, PR25 locked evaluation / leaderboard-entry execution from that retest evidence, PR26 paper-shadow outcomes from explicit observation inputs, PR27 resolver visibility for completed retest chains, PR28 research autopilot records for completed revision retests, PR29 CLI recording of those completed chains, PR30 UX visibility for the recorded completed-chain autopilot evidence, PR31 strategy lineage summaries, PR32 recursive multi-generation lineage summaries, PR33 revision tree rows, PR34 regressions for branching/missing-parent/cycle lineage edges, PR37 performance trajectory, PR38 verdicts, PR39 next research focus, PR40 machine-readable lineage JSON, and PR41 idempotent lineage-focus research agendas. | Needs strategy generation, self-evolving skills, CPCV-like paths, PBO, DSR, bootstrap, and parameter stability. |
| UI and operations | Shows read-only decisions, health, portfolio, risk, automation, broker/sandbox state, concrete strategy context, leaderboard state, paper-shadow attribution, next research action, pending revision retest scaffold state, visible next retest task plans, visible audit logs for task-plan inspections, visible completed revision retest autopilot runs, recursive strategy lineage summaries with parent/depth rows, escaped revision-change summaries, performance trajectory, human-readable trajectory verdict, next research focus, demotion/quarantine signals, a `strategy-lineage` JSON CLI for automation consumers, and PR42 lineage-derived research agenda visibility. | Needs richer comparison charts and denser strategy performance views. |
...
| PR32 | Recursive strategy lineage | Multi-generation revisions and their paper-shadow outcomes remain tied to the original root strategy in strategy UX. |
| PR33 | Strategy lineage tree visibility | Revision parent/depth rows make nested or branching strategy self-evolution inspectable in UX. |
| PR34 | Strategy lineage edge regressions | Branching revision trees, missing parents, and parent cycles have committed regression coverage. |
| PR35 | Strategy revision change summary | Revision lineage rows expose strategy name, status, hypothesis, source outcome, and intended failure attributions. |
| PR36 | Strategy revision escaping regressions | Malicious HTML fixtures prove revision names, hypotheses, source outcomes, and intended fixes remain escaped in UX. |
| PR37 | Strategy lineage performance trajectory | Lineage paper-shadow outcomes expose after-cost excess, delta, improvement/worsening label, action, and failure attributions in UX. |
| PR38 | Strategy lineage performance verdict | Lineage summaries expose a concise human verdict, improvement/worsening/unknown counts, latest action, and latest failure focus before raw trajectory rows. |
| PR39 | Strategy lineage next research focus | Lineage verdicts translate into a concrete next study direction for the self-evolving strategy loop. |
| PR40 | Strategy lineage CLI | The latest lineage summary is available as read-only JSON for automation and research consumers without HTML scraping. |
| PR41 | Strategy lineage research agenda | The latest lineage next research focus can be persisted as an idempotent research agenda for the next self-evolution loop. |
| PR42 | Strategy lineage agenda UX | Lineage-derived research agendas are visible beside lineage summaries in dashboard and operator console. |
