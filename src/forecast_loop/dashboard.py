...
def _latest_cross_sample_autopilot_run(
    ...
    lineage_ids = _lineage_strategy_ids(summary, task_plan)
    ...
    matches = [
        ...
        and _valid_cross_sample_autopilot_run(
            run,
            paper_shadow_outcomes=paper_shadow_outcomes,
...
def _latest_autopilot_run_for_agenda(
    ...
    if agenda is None:
        return None
    lineage_ids = _lineage_strategy_ids(summary, None)
    matches = [
        run
        for run in runs
...
def _valid_cross_sample_autopilot_run(
    ...
    *,
    ...
    if run.paper_shadow_outcome_id is None or run.paper_shadow_outcome_id != latest_outcome_id:
        return False
    outcome = _paper_shadow_outcome_by_id(paper_shadow_outcomes, run.paper_shadow_outcome_id)
...
def _paper_shadow_outcome_by_id(
...
def _strategy_card_by_id(cards: list[StrategyCard], card_id: str | None) -> StrategyCard | None:
    if card_id is None:
        return None
