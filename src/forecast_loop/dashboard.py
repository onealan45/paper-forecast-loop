...
def build_dashboard_snapshot(storage_dir: Path | str) -> DashboardSnapshot:
    ...
    return DashboardSnapshot(
        ...
        latest_portfolio_snapshot=_latest(portfolio_snapshots),
        latest_risk_snapshot=_latest(dashboard_risk_snapshots),
...
def _render_lineage_replacement_strategy(
    ...
    return f"""
      ...
      <div class="evidence-block">
        <h3>替代策略 Retest Scaffold</h3>
        <dl>
          <dt>Retest Trial</dt><dd><code>{escape(retest_plan.pending_trial_id if retest_plan and retest_plan.pending_trial_id else "尚未建立")}</code></dd>
          <dt>Dataset</dt><dd><code>{escape(retest_plan.dataset_id if retest_plan and retest_plan.dataset_id else "n/a")}</code></dd>
    ...
    return f"""
      <div class="evidence-block">
        ...
        <dl>
          ...
          <dt>Failure Attribution</dt><dd>{_dashboard_failure_attribution_counts_inline(summary.failure_attribution_counts)}</dd>
...
def _dashboard_failure_attribution_counts_inline(values: dict[str, object]) -> str:
    if not values:
        return '<span class="empty">none</span>'
    return "；".join(
        f"<code>{escape(_display_failure_attribution_code(str(key)))}</code>={escape(_dashboard_value(value))}"
        for key, value in sorted(values.items(), key=lambda item: str(item[0]))
    )
