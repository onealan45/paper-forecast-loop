...
def _strategy_lineage_performance_trajectory(summary: StrategyLineageSummary | None) -> str:
    ...
    return _plain_list(
        [
            ...
            f" / {node.change_label}"
            f" / Action {_display_research_action(node.recommended_strategy_action)}"
            f" / Failures {_display_failure_attribution_codes(node.failure_attributions)}"
...
def _action_count_rows(values: dict[str, object]) -> str:
    if not values:
        return '<p class="muted">none</p>'
    rows = "".join(
        "<tr>"
        f"<td><code>{escape(_display_research_action(str(key)))}</code></td>"
        f"<td>{escape(_format_artifact_value(value))}</td>"
        "</tr>"
        for key, value in sorted(values.items(), key=lambda item: str(item[0]))
    )
...
def _action_count_labels(values: dict[str, object]) -> list[str]:
    return [
        f"{_display_research_action(str(key))}: {_format_artifact_value(value)}"
        for key, value in sorted(values.items(), key=lambda item: str(item[0]))
    ]
...
def _strategy_research_digest_panel(
  ...
  <article class="panel wide">
    ...
    <h4>決策阻擋研究證據</h4>
    {_digest_decision_research_evidence(digest, evidence)}
    <p>證據：</p>
...
def _digest_decision_research_evidence(
    ...
    unresolved = (
        "<p class=\"muted\">未解析 artifact：</p>" + _artifact_id_list(unresolved_ids)
        if unresolved_ids
        else ""
    )
    return (
