...
def test_display_step_artifact_adds_readable_copy_without_losing_codes():
    ...
    blocked_run = display_step_artifact("next_task_blocked_reason", "cross_sample_autopilot_run_blocked")
    ...
    assert blocked_run == "cross-sample autopilot run 被證據鏈阻擋 (cross_sample_autopilot_run_blocked)"
    assert missing == (
