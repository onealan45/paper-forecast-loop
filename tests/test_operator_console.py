...
def _seed_visible_strategy_research_digest(repository: JsonFileRepository, now: datetime) -> None:
    ...
    repository.save_event_edge_evaluation(
        EventEdgeEvaluation(
            evaluation_id="event-edge:visible-blocker",
            event_family="crypto_flow",
            event_type="CRYPTO_FLOW",
            symbol="BTC-USD",
            created_at=now,
    ...
    repository.save_backtest_result(
        BacktestResult(
            result_id="backtest-result:visible-blocker",
            backtest_id="backtest-run:visible-blocker",
            created_at=now,
            symbol="BTC-USD",
            start=now - timedelta(days=30),
            end=now,
    ...
    repository.save_walk_forward_validation(
        WalkForwardValidation(
            validation_id="walk-forward:visible-blocker",
            created_at=now,
            symbol="BTC-USD",
            start=now - timedelta(days=20),
            end=now,
            ...
            backtest_result_ids=["backtest-result:visible-blocker"],
            windows=[],
            decision_basis="decision blocker fixture",
        )
    )
